...
router.get('/google', (req, res) => {
    try {
      ...
      `);
        ...
        const authUrl = google_auth_1.googleAuth.getAuthUrlForClient(clientId);
        logger_1.logger.info('Redirecting to Google OAuth', { clientId, authUrl });
        res.send(`
      <!DOCTYPE html>
      <html>
        <head>
    ...
    `);
        ...
        logger_1.logger.error('Error initiating OAuth flow', error);
        res.status(500).json({
            success: false,
...
router.get('/oauth2callback', async (req, res) => {
    try {
        ...
        const state = req.query.state; // Contains client_id
        if (!code) {
            logger_1.logger.warn('No authorization code received');
            res.status(400).send(`
        <!DOCTYPE html>
        <html>
          <head>
      ...
      `);
        ...
        if (!state) {
            logger_1.logger.warn('No client ID in state parameter');
            res.status(400).send(`
        <!DOCTYPE html>
        <html>
          <head>
      ...
      `);
        ...
        await google_auth_1.googleAuth.getTokenFromCodeForClient(code, state);
        logger_1.logger.info('OAuth flow completed successfully', { clientId: state });
        res.send(`
      <!DOCTYPE html>
      <html>
        <head>
    ...
    `);
        ...
        logger_1.logger.error('Error in OAuth callback', error);
        res.status(500).send(`
      <!DOCTYPE html>
      <html>
        <head>
