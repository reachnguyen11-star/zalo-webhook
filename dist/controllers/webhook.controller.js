    ...
    async handleZaloWebhook(req, res, next) {
        try {
            ...
            if (!req.body || Object.keys(req.body).length === 0) {
                logger_1.logger.info('Webhook verification request (empty body)', { clientId });
                res.status(200).json({
                    success: true,
            ...
            if (!formDataRaw || !formDataRaw.fields || formDataRaw.fields.length === 0) {
                ...
                logger_1.logger.info('No form data in webhook event (possibly verification)', { clientId, body: req.body });
                res.status(200).json({
                    success: true,
            ...
            const sheetsService = new sheets_service_1.SheetsService(oauth2Client, client.googleSheetId, clientId, client.sheetName // Pass sheet name (defaults to 'Sheet1' if not specified)
            );
            // Check for duplicate
